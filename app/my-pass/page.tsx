...
function MyPassContent() {
  ...
  useEffect(() => {
    const code = searchParams.get("code") ?? sessionStorage.getItem("kumbh-pass") ?? "";
    if (code) {
      setPassCode(code);
      fetchPass(code);
    }
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-8 sm:px-6">
      <h1 className="text-2xl font-bold text-gray-900">My Parking Pass</h1>
      <p className="text-kumbh-700">मेरा पार्किंग पास</p>
      ...
      {loading && <p className="mt-6 text-center text-gray-500">Loading pass...</p>}
      {error && (
        ...
        <div className="mt-8 space-y-6">
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-500">Parking Zone</p>
                <p className="font-semibold">{zone.name}</p>
                <p className="text-kumbh-700">{zone.nameHindi}</p>
              </div>
              <div>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm">
            ...
            <p className="text-amber-900">
              If your vehicle is mis-parked, you will receive SMS and auto-call at{" "}
          ...
          <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm">
            ...
            <p className="text-blue-900">
              Shuttle stop: <strong>{zone.shuttleStop}</strong> — show this QR at boarding.
            </p>
          </div>
        </div>
      )}
